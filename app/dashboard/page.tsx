  ...
  return (
    <>
      ...
      {enrolledCourses.length === 0 ? (
        <EmptyState title="No courses purchaged" description="You have not purchased any courses yet" buttonText="Browse Courses" href="/courses"/>
      ) : (
      ...
      <div className="flex flex-col gap-2 mb-5">
        <h1 className="text-3xl font-bold">Availabe Courses</h1>
      ...
      {courses.filter((course) =>!enrolledCourses.some(({Course:enrolled}) =>enrolled.id === course.id)).length === 0 ? (
        <EmptyState title="No courses available" description="There are no courses available at the moment" buttonText="Browse Courses" href="/courses"/>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {
