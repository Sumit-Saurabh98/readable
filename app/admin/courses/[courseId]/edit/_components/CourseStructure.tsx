...
export function CourseStructure({ data }: iAppProps) {
  const initialItems =
    data.chapter.map((chapter) => ({
      ...
      isOpen: true, // default chpater to open
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (activeType === "chapter") {
      ...
      if (oldIndex === -1 || newIndex === -1) {
        toast.error("Could not find chapter old/new indec for reordering");
        return;
      }
    ...
    if (activeType === "lesson" && overType === "lesson") {
      ...
      if (oldLessonIndex === -1 || newLessonIndex === -1) {
        toast.error("Could not find the lession for reordring.");
        return;
      }
      ...
      if (courseId) {
        ...
        toast.promise(reorderLessonPromise(), {
          ...
          error: () => {
            setItems(previousItems);
            toast.error("Could not reorder the lessons");
            return "Could not reorder the lessons";
          },
        });
      }
                      ...
                      <div className="flex items-center justify-between border-b border-border px-4 py-2">
                        <div className="flex items-center gap-2">
                          <Button
                            variant={"ghost"}
                            size={"icon"}
                            className="cursor-grab opacity-60 group-hover:opacity-100"
                            {...listeners}
                          >
